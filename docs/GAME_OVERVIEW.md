...
Источники: `README.md`, `docs/game_concept.json`, `src/app/App.tsx`, `src/screens/*`, `src/content/*`, `src/shared/store/gameStore.ts`, `src/shared/lib/progression.ts`, `src/entities/save/schema.ts`, `ANALYTICS_EVENTS.md`.
...
- 4 коллекционных артефакта северной кампании.
...
## 2. Первый запуск и первый экран
...
- отправляет `game_ready` и `screen_view: home`.

После загрузки игрок попадает не на отдельный маркетинговый экран, а сразу в экран выбора экспедиции.

Первый экран показывает:

- логотип/шапку "Expedition Mysteries";
- общий прогресс по всем кампаниям;
- кнопку настроек;
- заголовок "Choose Your Expedition";
- три карточки кампаний;
- статус каждой кампании: новая, в процессе, завершена, заблокирована;
- прогресс кампании `done / total`;
- последнюю пройденную точку маршрута;
- подсказку, почему следующая кампания заблокирована;
...
На мобильном экране кампании представлены как карусель карточек. На desktop - как ряд карточек с линией маршрута между экспедициями.
...
3. Игрок видит экран выбора экспедиции.
4. Игрок выбирает доступную кампанию.
5. Открывается экран карты/журнала уровней кампании.
6. Игрок запускает текущий доступный уровень или переигрывает уже пройденный.
7. Открывается gameplay screen с парой изображений A/B.
8. Игрок ищет все отличия, нажимая по любому из двух снимков.
9. Каждое найденное отличие отмечается и сохраняется.
...
13. Игра сохраняет результат, награды и прогресс.
14. Игрок может перейти к следующему уровню, переиграть уровень или вернуться на карту.
15. При некоторых completion-событиях может появиться review prompt или interstitial.
16. После завершения кампании открывается следующая кампания.
...
- начисляются rewards;
- открываются артефакты, если milestone выполнен;
- очищается `inProgress`;
- показывается CTA на следующий уровень, retry или карту.
...
В текущей реализации есть 4 артефакта, привязанных к White Meridian:

| Артефакт | Открытие |
|---|---:|
| Brass compass | после уровня White Meridian 3 |
| Field radio | после уровня White Meridian 6 |
| Pressed blue flower | после уровня White Meridian 9 |
| Torn route map | после уровня White Meridian 12 |
...
Collection screen существует и показывает 4 слота, но в текущем основном UI нет явной кнопки входа в коллекцию. Это нужно учитывать как недоделанный entry point.
...
Как и у коллекции, route для перехода на Daily есть в store/App, но явного entry point в актуальном Home/Map UI не найдено.
...
- открывает 4 коллекционных артефакта;
...
| 3 | White Gull Lighthouse | Маяк и навигационные детали. Первый заметный milestone коллекции. | 5 | 1 | 130 AP, +1 magnifier, Brass compass |
...
| 6 | Glacier Camp | Лагерь у ледника. Сцена про полевое снаряжение, лыжи, следы, флаги и грузовые элементы. | 6 | 2 | 160 AP, +1 magnifier, Field radio |
| 7 | Old Mountain Tunnel | Старый горный тоннель. Игрок ищет ставни, лампы, двери, ящики и цветовые акценты. | 7 | 2 | 170 AP |
| 8 | Observatory Dome | Купол обсерватории. Более плотная композиция с телескопом, часами, люком, книгой и мебелью. | 7 | 2 | 180 AP |
| 9 | Polar Greenhouse | Полярная теплица. Фокус на растениях, емкостях, приборах, поддонах и синем цветке. | 7 | 2 | 190 AP, +1 magnifier, Pressed blue flower |
| 10 | Radio Relay Tower | Радиорелейная башня. Сложнее из-за приборных и сигнальных деталей, кабелей и индикаторов. | 7 | 3 | 200 AP |
| 11 | Living Module | Жилой модуль экспедиции. Мелкие бытовые детали: полки, шахматная фигура, одежда, ботинки. | 7 | 3 | 210 AP |
| 12 | Hidden Valley Archive | Скрытый архив долины. Финальный архивный узел с радиостанцией, компасом, цветком, картой и ящиками. | 8 | 3 | 220 AP, +1 magnifier, Torn route map |
| 13 | Ice Cliff Camp | Лагерь у ледяного обрыва. Эпилог северного маршрута с палаткой, мостом, веревками, приборами и дальними деталями. | 9 | 3 | 230 AP |
...
| 3 | Dune Basin | Чаша дюн, камни и входы в полевые точки. Акцент на маркерах, лампах, скальных формах и инструментах. | 9 | 1 | 170 AP, +1 magnifier |
...
| 6 | Canyon Pass | Каньонный проход. Больше вертикальных форм, лестниц, флагов, генераторов, лебедок и цепей. | 8 | 2 | 200 AP, +1 magnifier |
...
| 8 | Oasis Iram | Оазис и водная инженерия. Отличия вокруг акведука, мостика, водяных ворот, бутылок, насоса и мерной рейки. | 8 | 2 | 220 AP |
...
| 10 | Ruined Observatory | Разрушенная обсерватория. Игрок ищет дверные, каменные, посудные и навесные детали среди руин. | 9 | 3 | 240 AP |
...
| 13 | Buried Meridian | Погребенный меридиан. Финал пустынной линии: таблички, карты, проходы, ткани, веревки и стол с находками. | 8 | 3 | 270 AP |
...
| 3 | Botanical Camp | Лагерь ботаников. Микроскопы, колбы, specimen row, гербарии, обувь и полевые карты. | 8 | 1 | 210 AP, +1 magnifier |
...
| 6 | Stilt Village | Деревня на сваях. Сцена с хижинами, картой, корзинами, гамаком, буйками, веслом и аптечкой. | 9 | 2 | 240 AP, +1 magnifier |
...
| 8 | Temple of Roots | Храм корней. Темные проемы, рельефы, фонарь, компас, кисть, трещины пола и водные следы. | 9 | 2 | 260 AP |
...
| 10 | Canopy Observatory | Обсерватория над кронами. Телескопы, гномон, озеро, стойки, компасная роза и водяная чаша. | 10 | 3 | 280 AP |
...
| 13 | Evacuation Airstrip | Эвакуационный аэродром. Финальная точка с самолетом, радиосвязью, ящиками, картой, шлангами и лагерем. | 9 | 3 | 310 AP |
...
- Daily и Collection screens существуют, но не имеют очевидного entry point в текущем Home/Map UI.
- Archive points записаны как rewards в level data, но не представлены как отдельный баланс в save/metagame.
- Артефакты есть только для White Meridian; Sand/Emerald не имеют своих коллекционных наборов.
...
- Нет отдельного narrative intro/pre-level story screen; сюжет в основном передается через названия, сеттинг и маршрут.

## 11. Как игру развивать дальше

### Первый приоритет: закрыть core loop

1. Добавить явные entry points для Daily и Collection на первом экране или карте.
2. Решить продуктово, остается ли 5-минутный timer в campaign mode.
3. Сделать post-level reward более понятным: что игрок получил, что открыл, что изменилось на маршруте.
4. Добавить artifact reveal flow после milestone-уровней.
5. Привести archive points к решению: удалить из rewards, если не нужны, или превратить в видимую мета-ценность.

### Второй приоритет: усилить мету

1. Добавить коллекции для Sand Meridian и Emerald Meridian.
2. Сделать Collection не просто списком статусов, а альбомом с описанием находок.
3. Добавить narrative snippets к уровням: короткая архивная заметка перед стартом или после победы.
4. Развить daily streak: 1 день, 2 день, 3 день, missed day, already claimed.
5. Добавить campaign completion screen, который подводит итог экспедиции и открывает следующую.
