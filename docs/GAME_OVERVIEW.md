...
- Startup routing: новый игрок сразу попадает в первый уровень с onboarding, возвращающийся - в in-progress или следующий уровень, полностью завершивший контент - в коллекцию.
...
После загрузки игра не всегда открывает home screen. `openStartupScreen()` выбирает точку входа по save:

- если есть `inProgress`, игрок возвращается прямо в этот уровень;
- если это новый save, запускается первый уровень White Meridian с мягким onboarding overlay;
- если есть следующий непройденный campaign level, игра открывает его напрямую;
- если все campaign levels завершены, игрок попадает в коллекцию.
...
3. Startup routing открывает in-progress/следующий уровень, коллекцию после полного прохождения или первый уровень для нового игрока.
...
17. После завершения кампании открывается следующая кампания; после завершения всех кампаний startup ведет в коллекцию.
