...
- две лупы или rewarded;
- показывает одно точное отличие;
- после успешного rewarded применяется автоматически;
- после ошибки рекламы игрок возвращается в уровень без потерь.
