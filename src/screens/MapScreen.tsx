...
function LockIcon({ size = 16, color = "#879087" }: { size?: number; color?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <rect x="5" y="11" width="14" height="9" rx="1.8" stroke={color} strokeWidth="1.8" />
...
function CheckIcon({ size = 12 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M20 6L9 17L4 12" stroke="#1A130A" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}

function PlayIcon({ size = 12, color = "#1A130A" }: { size?: number; color?: string }) {
  return (
...
function StarIcon({ filled }: { filled: boolean }) {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill={filled ? "#D8AF63" : "rgba(213,195,154,0.16)"} aria-hidden="true">
...
export function MapScreen() {
  ...
  const setReviewNativeResolved = useGameStore((state) => state.setReviewNativeResolved);
  const setReviewUnavailableReason = useGameStore((state) => state.setReviewUnavailableReason);
  const mountedChapterIdRef = useRef<ChapterId>(screen.kind === "map" ? screen.chapterId : "northern-route");
  const chapterId = screen.kind === "map" ? screen.chapterId : mountedChapterIdRef.current;
  ...
  const analyticsPayload = useMemo(() => ({
    ...
    deviceType: window.innerWidth < 768 ? "mobile" : window.innerWidth < 1280 ? "tablet" : "desktop",
    language: saveData.settings.locale,
    promptOrdinal
  }), [chapter.id, completedLevelsCount, promptOrdinal, saveData.settings.locale]);

  const levelCards = useMemo<LevelCardState[]>(() => chapter.levels.map((level) => {
  ...
  const currentLevel = levelCards.find((level) => level.current) ?? levelCards[levelCards.length - 1];
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      void (async () => {
        ...
        if (checkRunRef.current !== checkId || useGameStore.getState().screen.kind !== "map") {
          return;
        }
  ...
  async function handleReview() {
    if (reviewSubmitGuardRef.current || isSubmittingReview || reviewPromptRuntime.nativeRequestInFlight) return;
      ...
      <div className="relative z-10 min-h-screen">
        <div className="map-topbar flex items-center gap-3 border-b border-[#D5C39A]/10 px-5 pb-4 pr-20 pt-3 md:px-10 md:pr-24">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[8.5px] font-bold uppercase tracking-[.18em] text-[#B88A45]">{t("campaigns.campaignLabel")}</p>
            <h1 className="truncate font-cormorant text-[28px] font-semibold leading-none md:text-[34px]">{t(chapter.titleKey)}</h1>
          </div>
          <div className="hidden min-w-[110px] flex-col items-end gap-1 md:flex">
            <span className="text-[12px] font-bold text-[#D8AF63]">{chapterCompleted} / {chapter.levels.length}</span>
        ...
        <div className="map-content mx-auto w-full max-w-[1460px]">
            ...
            {levelCards.map((level) => (
              ...
              >
                ...
                <div className="map-level-body flex flex-1 flex-col px-4 py-3">
                  ...
                  <h3 className={`mt-1 font-cormorant text-[24px] font-semibold leading-[1.05] ${
                  ...
                  <div className="mt-auto">
                    {level.completed && (
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10.5px] font-semibold text-[#879087]">{t("campaigns.replay")}</span>
                      </div>
                    )}
                    {level.current && (
                    ...
                    {level.locked && (
                      <div className="flex items-center gap-2">
                        <LockIcon size={12} color={level.order === currentLevel.order + 1 ? "#B88A45" : "#879087"} />
          ...
          <div className="grid gap-3 px-4 pb-32 pt-5 md:hidden">
          {levelCards.map((level) => (
            <button
              key={level.id}
              type="button"
              ...
              onClick={() => startLevel(level.id)}
              aria-label={`${t("campaigns.levelCode", { order: level.order })} ${level.title}`}
              className={`flex h-[152px] flex-col overflow-hidden rounded-[12px] bg-[#222A25] text-left ${
                level.current
            ...
            >
              ...
              <div className="flex flex-1 flex-col px-3 py-2">
                <p className={`text-[7.5px] font-bold uppercase tracking-[.14em] ${
                  level.current ? "text-[#D8AF63]" : level.completed ? "text-[#B88A45]" : "text-[#879087]"
                }`}>
                  {level.current
                    ? t("campaigns.currentLevel")
                ...
                <h3 className={`mt-1 font-cormorant text-[19px] font-semibold leading-[1.05] ${
                  level.locked ? "text-[#D5C39A]/55" : "text-[#D5C39A]"
                }`}>
                  {level.title}
                </h3>
                <div className="mt-auto">
                  ...
                  {level.locked && (
                    <span className="text-[9.5px] font-semibold text-[#879087]">{level.footerLabel}</span>
                  )}
                </div>
              </div>
            </button>
          ...
          <div className="mx-auto flex max-w-sm items-center gap-3">
            <div className="min-w-0 flex-1">
              <p className="text-[8.5px] font-bold uppercase tracking-[.16em] text-[#B88A45]">
                {t("campaigns.currentLevelSummary", { progress: `${chapterCompleted} / ${chapter.levels.length}` })}
              </p>
              <p className="truncate font-cormorant text-[18px] font-semibold text-[#D5C39A]">{currentLevel.title}</p>
            </div>
            <button
          ...
          <div className="mx-auto hidden w-full max-w-[1460px] md:block">
            <footer className="mt-4 flex items-center justify-center gap-2 border-t border-[#D5C39A]/10 bg-[rgba(16,21,18,.5)] py-3 text-[10.5px] text-[#879087]">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <path d="M20 6L9 17L4 12" stroke="#879087" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              {t("campaigns.autoSave")} · {t("campaigns.levelsDone", { done: chapterCompleted })}
            </footer>
          </div>
        </div>
      </div>
