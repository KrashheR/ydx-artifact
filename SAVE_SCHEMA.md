...
- `magnifiers` (hints) are capped at `MAX_MAGNIFIERS = 5`. The cap applies when granting level/daily rewards and when loading or migrating a save, so older saves with a larger stock clamp down to 5.
