...
const LEGACY_GOAL_NAMES: GoalDefinition[] = [
  ...
  {
    name: "Клик по закрытой кампании",
    identifier: "aa_locked_campaign_clicked",
  },
  ...
  {
    name: "Оффер rewarded-подсказки открыт",
    identifier: "aa_rewarded_hint_offer_opened",
  },
  {
    name: "Rewarded-подсказка запрошена",
    identifier: "aa_rewarded_hint_requested",
  },
  {
    name: "Rewarded-подсказка выдала награду",
    identifier: "aa_rewarded_hint_rewarded",
  },
  {
    name: "Rewarded-подсказка закрыта без награды",
    identifier: "aa_rewarded_hint_closed",
  },
  {
    name: "Rewarded-подсказка не загрузилась",
    identifier: "aa_rewarded_hint_failed",
  },
  {
    name: "Interstitial подходит по условиям",
    identifier: "aa_interstitial_eligible",
  },
  ...
  {
    name: "Review prompt: нажата оценка",
    identifier: "aa_review_prompt_review_clicked",
  },
  {
    name: "Review prompt: нажато позже",
    identifier: "aa_review_prompt_later_clicked",
  },
  ...
  {
    name: "Native review закрыт без отправки",
    identifier: "aa_review_native_closed",
  },
  {
    name: "Native review недоступен",
    identifier: "aa_review_native_unavailable",
  },
...
function getCounterId() {
  return (
    process.env.YANDEX_METRICA_COUNTER_ID ?? process.env.VITE_YANDEX_METRICA_ID
  );
}
...
async function fetchExistingGoals(counterId: string, token: string) {
  const response = await requestJson<GoalsResponse>(
...
async function main() {
  ...
  if (!token) {
    console.log(
      "No YANDEX_OAUTH_TOKEN found. Showing local goal list only; API sync requires an OAuth token.",
    );
    printGoalList(GOALS);
  ...
  const existingIdentifiers = new Set(
    existingGoals.map(getGoalIdentifier).filter(Boolean),
  );
  const missingGoals = GOALS.filter(
    (goal) => !existingIdentifiers.has(goal.identifier),
  );
