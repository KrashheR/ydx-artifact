...
Motion/VFX uses runtime CSS tokens in `src/app/styles.css` and typed timings in `src/shared/motion/motion.ts`. `useReducedEffects` combines the operating-system `prefers-reduced-motion` preference with `saveData.settings.reducedMotion`; effects remain decorative, while game state, saving and navigation are never animation callbacks. Gameplay feedback stays local to its state owner: `PhotoComparator` renders found/miss/hint markers and the mobile A/B flip/slider cues, while `GameScreen` owns HUD progress, timer warnings and the final-find delay. Mobile players can also choose side-by-side photos; all three compare schemes share the same camera state where both images are visible. The 30-second timer cue plays once; the final-10-second cue is a restrained two-second rust rhythm that stops when gameplay is blocked. Marker geometry and animated ring transforms are intentionally split into nested elements so rotated ellipse/polygon hitboxes retain their visual orientation.

Meta-surface and rare-reward motion use the same CSS tokens: Home/Map state accents are finite, `ArtifactRevealOverlay` retains its staged seal reveal, and `CampaignCaseReportModal` uses shared classes rather than component-local keyframes. `App` pauses decorative CSS animations while the platform is paused or the document is hidden; this is deliberately independent of gameplay/save lifecycle.

`src/features/gameplay/PhotoComparator.tsx` displays scene images with `object-fit: contain`. The comparator measures the contained image plane inside each photo panel and converts pointer, found-marker, hint-marker and wrong-click coordinates through that plane, so normalized hitboxes stay aligned when the panel aspect ratio adds letterbox space. Optional mobile-only A/B registration offsets come from the validated `src/content/sceneAlignment.json`; the offset moves that complete image/interaction plane in flip, slider and side-by-side modes while desktop remains unchanged. `pnpm dev:align` is the dev-only authoring surface and writes this source through a serve-only Vite endpoint.
