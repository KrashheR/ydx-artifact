...
- `pnpm dev:validate` starts the same layout-debug mode through Vite dev/HMR, so reviewers can compare authored hitboxes against every level's `3.*` markup reference while code and content edits update live. In this mode, markers can be dragged directly on the scene; adjusted coordinates apply immediately, are stored per level in localStorage, and can be exported with the on-screen "Copy JSON" button as a replacement `differences` array for the relevant `src/content/*` module. Use "Reset" to discard local authoring overrides for the level.
