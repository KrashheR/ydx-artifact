...
- Level 13 hitboxes were transcribed from `public/assets/scenes/northern-route/13/3.webp` for the tent opening, survey device, mug, rope coil, tarp, rope marker, striped post, bridge and seagull differences. The A/B pair was restored to its first committed runtime WebP state (`61b1f77`) with the other gameplay scene pairs; in that historical state `1.webp` and `2.webp` are identical, so content validation reports the level until the later A-scene replacement is reapplied.
