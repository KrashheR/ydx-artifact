...
- [x] Published all 30 missing goals from the then-68-event typed registry to
  the configured production counter on 2026-07-21. Re-run
  `pnpm metrika:goals` before release and after any registry change (including
  the current 69th `setting_interest_selected` goal), then require `Missing: 0`.
...
- [x] Added the optional unrewarded post-campaign setting-interest survey and
      `setting_interest_selected` on 2026-07-21; campaign exposure/eligibility/entry/
      completion remains available for comparison, but behavioral data alone is not a
